...
<html lang="es">
    ...
    <body>  
        ...
        <div class="Cprincipal_index card-panel grey lighten-4">
            ...
            <br>
            <div>
                <h5 class="center-align">Índices de reprobación Ingeniería Mecánica</h6>
            </div>
            <br>
            <br>
            ...
            <form class="col s12 m12 l12 center-align">
                <select id="ListaInidce" name="ListaInidce" onchange="window.open(this.options[this.selectedIndex].value,'_blank')" class="center-align">
                    <?php
                        ...
                        require_once("BD.php");
                        $db = new BD();
                        $SQL = "SELECT periodo, fecha FROM indices WHERE carrera='Mecanica'";
                        $resultado = $db->ejecutar($SQL);
                        foreach($resultado as $fila){
                    ?>
                        <option value="admin/Indices/Mecanica/<?=$fila[0]?>_<?=$fila[1]?>.pdf"><?=$fila[0]?> <?=$fila[1]?></option>
                        <h4><?=$fila[0]?></h4>
                    <?php
                        }
                    ?>
