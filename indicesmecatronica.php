...
<html lang="es">
    ...
    <script type="text/javascript" src="https://code.jquery.com/jquery-3.2.1.min.js"></script>
        ...
        <div class="Cprincipal_index card-panel grey lighten-4">
            ...
            <br>
            <div>
                <h5 class="center-align">Índices de reprobación Ingeniería Mecatrónica</h6>
            </div>
            <br>
            <br>
            ...
            <form class="col s12 m12 l12 center-align">
                <select id="ListaInidce" name="ListaInidce" onchange="window.open(this.options[this.selectedIndex].value,'_blank')" class="center-align">
                    <?php
                        ...
                        require_once("BD.php");
                        $db = new BD();
                        $SQL = "SELECT periodo, fecha,url FROM indices WHERE carrera='Mecatronica'";
                        $resultado = $db->ejecutar($SQL);
                        foreach($resultado as $fila){
                    ?>
                        <option value="admin/Indices/Mecatronica/<?=$fila[0]?>-<?=$fila[1]?>.pdf"><?=$fila[0]?>-<?=$fila[1]?></option>
                        <h4><?=$fila[0]?></h4>
                    <?php
                        }
                    ?>
